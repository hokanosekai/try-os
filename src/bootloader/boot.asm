...
.search_kernel:
  ...
  ; if no kernel found, we check the next entry
  ...
  ; if kernel not found, print error message and halt
  jmp kernel_not_found              ; jump to kernel_not_found

.kernel_found:
  ; di points to kernel file entry
  mov ax, [di + 26]                 ; ax = di + 26 (LBA of kernel file)
  mov [kernel_cluster], ax          ; kernel_cluster = LBA of kernel file
